; ****************************************************************************
; wavplay2.s (for TRDOS 386)
; ----------------------------------------------------------------------------
; WAVPLAY2.PRG ! VIA VT8237 .WAV PLAYER program by Erdogan TAN
;
; 17/03/2017
;
...
write_audio_dev_info:
	...
	; DL  = Message color (07h = light gray, 0Fh = white) 
     	sys 	_msg, msgAudioDevInfo, 255, 0Fh
        retn
...
write_ac97_pci_dev_info:
	...
	and	bl, 0Fh
	mov	al, [ebx+hex_chars]
	mov	[msgBaseAddr+3], al
	mov	bl, dl
	shr	bl, 4
	mov	al, [ebx+hex_chars]
	mov	[msgBaseAddr+2], al
	mov	bl, ah
	mov	dl, bl
	and	bl, 0Fh
	mov	al, [ebx+hex_chars]
	mov	[msgBaseAddr+1], al
	mov	bl, dl
	shr	bl, 4
	mov	al, [ebx+hex_chars]
	mov	[msgBaseAddr], al
...
Credits:
	...
	db	'July 2020.',10,13,0
	db	'14/10/2017', 10,13,0
	db	'28/07/2020', 10,13,0 

msgAudioDevInfo:
	db 	'for VIA VT8233 Audio Controller.', 10,13,0
